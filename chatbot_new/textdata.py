...
class TextData:
    ...
    def __init__(self, args):
        ...
        self.corpusDir = os.path.join(self.args.rootDir, 'data/sogou/')
    ...
    def deco2sentence(self, decoderOutputs):
        ...
        print (decoderOutputs[0].shape)
        print (decoderOutputs)
        for out in decoderOutputs:
            sequence.append(np.argmax(out))  # Adding each predicted word ids

            #Ranyu add code; sample with temperature
            '''def sample(out,temperature=1.0):
                #Ranyu add variables
                prob_array = []
                exp_sum = 0
                exp_sum = sum(out)
                for ele in out:
                    prob_array.append(ele/exp_sum)
                preds = np.asarray(prob_array).astype('float64')
            ...
            #sequence.append(sample(out[0]))  # Adding each predicted word ids
            import heapq
            result_array = heapq.nlargest(10,out[0])
            output = result_array[rand_num]
            sequence.append(list(out[0]).index(output))  # Adding each predicted word ids
            #end Ranyu code'''
